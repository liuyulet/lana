package com.lana.base.mybatis.interceptor;
    ...
    public void beforeQuery(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) {
       ...
       /* boolean flagValue = false;
        if(parameter instanceof Map){
            ...
            if(paramMap.containsKey("DataScopeIgnore")){
                Object dataScopeIgnore = paramMap.get("DataScopeIgnore");
                if (dataScopeIgnore != null && dataScopeIgnore instanceof Boolean) {
                    flagValue = (Boolean) dataScopeIgnore;
                }
            }
        }
        if(flagValue){
            ...
            if (dataScopeList == null) {
                ...
                // 根据参数获取数据权限范围
                String scope = getDataScope((Map<?, ?>) parameter);
                // 如果没有数据权限范围或权限范围的SQL过滤器为空，则不进行后续处理
                // 不进行数据过滤
                if (scope == null || StrUtil.isBlank(scope)) {
                    return;
                }
                // 根据数据权限范围和原始SQL构建新的SQL语句
                // 拼接新SQL
                String buildSql = getSelectSql(boundSql.getSql(), scope);
                //System.out.println("=======================拼接新SQL======================:"+buildSql);
                // 修改BoundSql对象的SQL语句，使其包含数据权限范围的限制
                // 重写SQL
                PluginUtils.mpBoundSql(boundSql).sql(buildSql);
            }
        }*/
